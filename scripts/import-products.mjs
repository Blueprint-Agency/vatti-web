...
CATEGORIES.forEach((c, i) =>
  L.push(
    `INSERT INTO product_category (id, slug, name, h1, sort_order) VALUES ` +
      `(${i + 1}, ${q(c.slug)}, ${q(c.name)}, ${q(c.h1)}, ${i});`
  )
);
L.push("");
...
function imageFor(url, alt, width, height) {
  ...
  const id = ++imgSeq;
  imageId.set(url, id);
  // Pre-migration: url still points at WordPress so pages render today.
  // Phase 2 rewrites url to the R2 CDN and keeps legacy_url for the 301s.
  imageRows.push(
    `INSERT INTO image (id, url, legacy_url, alt, width, height) VALUES ` +
      `(${id}, ${q(url)}, ${q(url)}, ${q(alt)}, ${width ?? "NULL"}, ${height ?? "NULL"});`
  );
  return id;
}
  ...
  (p.downloads || []).forEach((d, n) =>
    body.push(
      `INSERT INTO product_download (product_id, position, label, url, kind) VALUES ` +
        `(${id}, ${n}, ${q(d.label)}, ${q(d.url)}, ${q(d.kind || "other")});`
    )
  );
