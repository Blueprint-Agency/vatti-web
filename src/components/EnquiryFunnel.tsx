...
export function EnquiryFunnel({
  ...
  const headingClass =
    "text-2xl font-semibold tracking-[-0.02em] outline-none sm:text-[1.75rem]";
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="mt-5 overflow-hidden rounded-sm border border-line bg-void">
        ...
        <div key={step} className="animate-panel p-5 sm:p-8">
          ...
          {question ? (
            ...
            <div className="mt-3">
              <h3 ref={headingRef} tabIndex={-1} id="funnel-name-heading" className={headingClass}>
                Your name
              </h3>
              <p className="mt-2 max-w-[48ch] text-ink-muted">
          ...
          <div className="mt-8 flex items-center justify-between gap-4 border-t border-line pt-5">
            ...
            {last ? (
              <p className="text-right text-sm text-ink-muted">
                That is everything. Your message is ready below.
              </p>
            ) : (
              <button
                type="button"
      ...
      {/* The message, in full, as it is being written. Under the card rather
          than beside it: on this page the card is the thing being looked at,
          and the text growing a line at a time underneath is what shows each
          answer landed. */}
      <div className="mt-6 rounded-sm border border-line bg-void p-5 sm:p-6">
